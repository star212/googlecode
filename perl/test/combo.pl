...
sub combo {
	...
	#unlink $filename or warn "failed on $filename:$!";
	rename "$filename.tmp", $filename.".html" or warn "failed on $filename:$!";
...
sub generateUrl {
	my($args) = "@_";
	my($b) = "";
	$args =~ s/http:\/\/res\.6677bank\.com//g;
	while($args =~ m/^\/([^\/]*)\/([^,]*,?)(\/?\1\/[^,]*,?)+$/) {
		$b .= "/$1";
		print $b."\n";
		$args =~ s/\/$1//g;
